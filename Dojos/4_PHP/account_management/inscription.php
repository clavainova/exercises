...
if ((!isset($_POST["email"]) || $_POST["email"] == "") || (!isset($_POST["pass"]) || $_POST["pass"] == "")) {
    ...
    $thisUser = new Utilisateur($_POST["email"], $hash, $_POST["username"]);
    if (!checkUnique($conn, $thisUser)) {
        $error = 109; //email or username already in use
...
redirect("http://localhost/progression/Dojos/4_PHP/index.php?page=settings"); //pass in settings homepage
?>